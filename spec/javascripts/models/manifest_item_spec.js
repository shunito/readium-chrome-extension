(function() {

  describe("Readium.Models.ManifestItem", function() {
    beforeEach(function() {
      spyOn(Readium, "FileSystemApi");
      spyOn(Readium.Models.ManifestItem.prototype, "resolvePath");
      this.man_item_attrs = {
        href: "Font/Helvetica-0850.otf",
        id: "id4",
        media_overlay: "",
        media_type: "image/svg+xml",
        properties: ""
      };
      return this.man_item = new Readium.Models.ManifestItem(this.man_item_attrs);
    });
    describe("initialization", function() {
      return it("works", function() {
        var man_item;
        man_item = new Readium.Models.ManifestItem(this.man_item_attrs);
        return expect(man_item.loadContent).toBeDefined();
      });
    });
    describe("isImage()", function() {
      it("is not an image if it is svg", function() {
        return expect(this.man_item.isImage()).toBeFalsy();
      });
      it("is not an image if it is xhmlt", function() {
        this.man_item.set("media_type", "application/xhtml+xml");
        return expect(this.man_item.isImage()).toBeFalsy();
      });
      return it("is an image if it has an image mime", function() {
        this.man_item.set("media_type", "image/jpeg");
        expect(this.man_item.isImage()).toBeTruthy();
        this.man_item.set("media_type", "image/png");
        return expect(this.man_item.isImage()).toBeTruthy();
      });
    });
    describe("isSvg()", function() {
      it("is svg if it has an svg mime", function() {
        return expect(this.man_item.isSvg()).toBeTruthy();
      });
      return it("is not svg if it deos not have svg mime", function() {
        this.man_item.set("media_type", "application/xhtml+xml");
        return expect(this.man_item.isSvg()).toBeFalsy();
      });
    });
    return describe("parseViewboxTag()", function() {
      beforeEach(function() {
        var parser, xml_string;
        xml_string = jasmine.getFixtures().read('manifest_item.svg');
        parser = new window.DOMParser();
        this.dom = parser.parseFromString(xml_string, 'text/xml');
        this.man_item = new Readium.Models.ManifestItem(this.man_item_attrs);
        return spyOn(this.man_item, "getContentDom").andReturn(this.dom);
      });
      it("loads the dom", function() {
        this.man_item.parseViewboxTag();
        return expect(this.man_item.getContentDom).toHaveBeenCalled();
      });
      return it("parses the tag", function() {
        var result;
        result = this.man_item.parseViewboxTag();
        expect(result.width).toEqual(368);
        return expect(result.height).toEqual(581);
      });
    });
  });

  describe("Readium.Models.SpineItem", function() {
    beforeEach(function() {
      spyOn(Readium, "FileSystemApi");
      spyOn(Readium.Models.SpineItem.prototype, "resolvePath");
      return this.spine_item_attrs = {
        href: "Content/pageNum-9.svg",
        id: "ch9",
        idref: "ch9",
        media_overlay: "",
        media_type: "image/svg+xml",
        page_prog_dir: "",
        properties: "",
        spine_index: 6
      };
    });
    describe("initialization", function() {
      it("works", function() {
        var spine_item;
        spine_item = new Readium.Models.SpineItem(this.spine_item_attrs);
        return expect(spine_item.isImage).toBeDefined();
      });
      it("calls loadContent() if the content is fixed layout", function() {
        var spine_item;
        spyOn(Readium.Models.SpineItem.prototype, "loadContent");
        spine_item = new Readium.Models.SpineItem(this.spine_item_attrs);
        return expect(Readium.Models.SpineItem.prototype.loadContent).toHaveBeenCalled();
      });
      return it("call does not call loadContent() if the content is reflowable", function() {
        var spine_item;
        spyOn(Readium.Models.SpineItem.prototype, "loadContent");
        spyOn(Readium.Models.SpineItem.prototype, "isFixedLayout").andReturn(false);
        spine_item = new Readium.Models.SpineItem(this.spine_item_attrs);
        return expect(Readium.Models.SpineItem.prototype.loadContent).not.toHaveBeenCalled();
      });
    });
    return describe("isFixedLayout()", function() {
      beforeEach(function() {
        this.spine_item = new Readium.Models.SpineItem(this.spine_item_attrs);
        return this.spine_item.set("media_type", "application/xhtml+xml");
      });
      it("is fixed layout it it is an image", function() {
        spyOn(this.spine_item, "isImage").andReturn(true);
        return expect(this.spine_item.isFixedLayout()).toBeTruthy();
      });
      it("is fixed layout if it is svg", function() {
        spyOn(this.spine_item, "isSvg").andReturn(true);
        return expect(this.spine_item.isFixedLayout()).toBeTruthy();
      });
      it("it defaults to checking what the books is", function() {
        var collection;
        collection = {
          isBookFixedLayout: jasmine.createSpy()
        };
        this.spine_item.collection = collection;
        this.spine_item.isFixedLayout();
        return expect(collection.isBookFixedLayout).toHaveBeenCalled();
      });
      return it("is fixed layout if its fixed_flow property is set", function() {
        this.spine_item.set("fixed_flow", true);
        return expect(this.spine_item.isFixedLayout()).toBeTruthy();
      });
    });
  });

}).call(this);
